...
# القاموس المحدث (تمت إضافة "المساحة M2" و "عدد الصكوك")
COLUMN_MAPPING = {
    # السعر
    'السعر': 'السعر', 'مبلغ الصفقة': 'السعر', 'Price': 'السعر', 'قيمة الصفقات': 'السعر', 
    'سعر الوحدة': 'السعر', 'Total Price': 'السعر',
    
    # المساحة (هنا كان النقص)
    'المساحة': 'المساحة', 'المساحة بالأمتار': 'المساحة', 'Area': 'المساحة', 
    'مساحة الوحدة': 'المساحة', 'Size': 'المساحة', 'المساحة M2': 'المساحة', 
    ...
    # إضافي
    'المطور': 'اسم_المطور', 'اسم المشروع': 'اسم_المشروع', 'عدد الصكوك': 'عدد_الصكوك'
}
        ...
        try:
                ...
                try:
                    ...
                    if 'MOJ' in file['name'].upper():
                        ...
                        for row in reader:
                            ...
                            # بحث مرن عن الأعمدة
                            if any(x in clean_row for x in ['السعر', 'قيمة الصفقات']) and \
                               any(x in clean_row for x in ['الحي', 'اسم الحي']):
                    ...
                    # المعالجة والتوحيد
                    # ---------------------------------------------
                    df_temp.columns = df_temp.columns.str.strip() # إزالة المسافات من أسماء الأعمدة
            ...
            if all_data:
                ...
                def classify(row):
                    ...
                    # تخمين بناءً على السعر والمساحة إذا كان النوع غير محدد
...
with st.sidebar:
    ...
    if 'bot' in st.session_state and hasattr(st.session_state.bot, 'df'):
        ...
        if not bot.df.empty:
            ...
            st.error("⚠️ لم يتم العثور على بيانات مطابقة.")

st.title("🧐 مدقق البيانات العقارية")

if 'bot' not in st.session_state:
    with st.spinner("جاري سحب ومعالجة البيانات من جميع الملفات..."):
        try:
...
if 'bot' in st.session_state and hasattr(st.session_state.bot, 'df'):
    ...
    if df.empty:
        st.warning("⚠️ لا توجد بيانات. تأكد من الملفات.")
    else:
        # الفلترة
        ...
        if sc2.button("تحليل 📊", use_container_width=True, type="primary") or search:
            ...
            if res.empty:
                ...
                if 'عدد_الصكوك' in res.columns: view_cols.append('عدد_الصكوك') # إضافة عمود عدد الصكوك إذا وجد
