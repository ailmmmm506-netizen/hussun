...
# 🌟 القاموس الشامل (يحتوي أسماء الملف الأول + أسماء الملف الثاني)
COLUMN_MAPPING = {
    # --- مفردات السعر ---
    'السعر': 'السعر',             # الملف الأول
    'مبلغ الصفقة': 'السعر',       # الملف الأول
    'قيمة الصفقات': 'السعر',      # الملف الثاني (الجديد)
    'Price': 'السعر',
    'سعر الوحدة': 'السعر',
    
    # --- مفردات المساحة ---
    'المساحة': 'المساحة',         # الملف الأول
    'المساحة بالأمتار': 'المساحة',# الملف الأول
    'المساحة M2': 'المساحة',      # الملف الثاني (الجديد)
    'Area': 'المساحة',
    'Size': 'المساحة',
    'مساحة الوحدة': 'المساحة',
    
    # --- مفردات الحي والمدينة ---
    'الحي': 'الحي',
    'اسم الحي': 'الحي',
    'المدينة': 'المدينة',
    'المنطقة': 'المدينة',         # الملف الثاني
    
    # --- مفردات نوع العقار ---
    'نوع العقار': 'نوع_العقار_الخام',      # موجود في الاثنين
    'تصنيف العقار': 'نوع_العقار_الخام',    # الملف الثاني
    'النوع': 'نوع_العقار_الخام',
    'الوحدة': 'نوع_العقار_الخام',
    
    # --- إضافات ---
    'عدد الصكوك': 'عدد_الصكوك',
    'المطور': 'اسم_المطور'
}
        ...
        try:
            ...
            for file in files:
                ...
                try:
                    ...
                    # 2. قراءة ذكية (تجربة الفواصل المختلفة)
                    try:
                        # نجرب الفاصلة المنقوطة (للوزارة)
                        df_temp = pd.read_csv(io.StringIO(content_str), sep=';', engine='python')
                        if len(df_temp.columns) <= 1: 
                            # إذا فشل، نجرب الفاصلة العادية
                            df_temp = pd.read_csv(io.StringIO(content_str), sep=',', engine='python')
                    except:
                        df_temp = pd.read_csv(io.StringIO(content_str), sep=None, engine='python')

                    if 'MOJ' in file['name'].upper(): source_type = 'عدل'

                    # 3. توحيد الأسماء (الخطوة الأهم)
                    df_temp.columns = df_temp.columns.str.strip() # تنظيف مسافات
                    df_temp.rename(columns=COLUMN_MAPPING, inplace=True) # تطبيق القاموس الشامل
                    df_temp = df_temp.loc[:, ~df_temp.columns.duplicated()] # حذف المكرر

                    # 4. التأكد من وجود البيانات الأساسية (السعر والمساحة)
                    # بعد التوحيد، يجب أن يكون لدينا عمود اسمه "السعر" وعمود "المساحة" بغض النظر عن الاسم القديم
                    if 'السعر' in df_temp.columns and 'المساحة' in df_temp.columns:
                        ...
                        if 'المدينة' in df_temp.columns:
                            ...
                            # نتأكد أننا نأخذ الرياض فقط، لكن بحذر (بعض الملفات قد لا تحتوي عمود مدينة)
                            df_temp = df_temp[df_temp['المدينة'].str.contains('الرياض', na=False)]
                        ...
                        if not df_temp.empty:
                            ...
                            print(f"⚠️ الملف {file['name']} فارغ بعد تنظيف الأرقام.")
                    else:
                        print(f"❌ الملف {file['name']} لا يحتوي على أعمدة السعر والمساحة المطلوبة.")

                except Exception as e:
                    print(f"❌ خطأ قراءة {file['name']}: {e}")
...
with st.sidebar:
    ...
    if 'bot' in st.session_state and hasattr(st.session_state.bot, 'df'):
        ...
        st.divider()
        st.markdown(f"**📂 الملفات المقروءة:**")
        
        if not bot.df.empty:
            # جدول يظهر كم صفقة سحبنا من كل ملف
            ...
            total_rows = len(bot.df)
            st.caption(f"إجمالي الصفقات المدمجة: {total_rows:,}")
        else:
            st.error("⚠️ لم يتم سحب بيانات.")

st.title("🧐 مدقق البيانات العقارية")

if 'bot' not in st.session_state:
    with st.spinner("جاري دمج البيانات من جميع المصادر..."):
        try:
...
if 'bot' in st.session_state and hasattr(st.session_state.bot, 'df'):
    ...
    if df.empty:
        st.warning("⚠️ لا توجد بيانات. تأكد من أن الملفات تحتوي على أعمدة السعر والمساحة.")
    else:
        # الفلترة
